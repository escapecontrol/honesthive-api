...
│   └── another-module/     # Repeat the structure for other domains
├── shared/
│   ├── application/        # Shared use cases (cross-domain)
│   ├── domain/             # Shared entities, value objects
│   ├── infrastructure/     # Shared integrations (e.g., database, logging)
...
> We're not running docker desktop. So we're using colima on the macos. In order to get started, we need to execute the following commands to ensure that the colima is started.
...
> _The invitation feature implementation has a limitation/potential bug. Once someone has been invited, the current implementation will remove the user from the invited list. We need to check if the email has already been invited to a team._
